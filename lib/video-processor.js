...
async function getProxyUrl(db, opts = {}) {
...
async function fetchYouTubeSegment(url, startSec, endSec, outPath, proxyUrl, cookiesFile, maxHeight = 1080) {
  ...
  // Multi-tier format selector.
  // PRIMARY: prefer separate 1080p video + best audio (DASH) — real HD quality; yt-dlp muxes internally via ffmpeg.
  // FALLBACKS: progressive (single-stream) formats. Guarantees we never fall below 480p unless nothing else works.
  const fmt = [
    // High-quality: separate streams (1080p+audio)
    `bestvideo[height<=${maxHeight}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=${maxHeight}]+bestaudio`,
    // Best single-stream at target height
    `best[height<=${maxHeight}][ext=mp4]`,
    `best[height<=${maxHeight}]`,
    // Graceful degradation for very-low-source content
    `best[height<=720][ext=mp4]`,
    `best[height<=720]`,
    `best[height<=480]`,
    `best[ext=mp4]`,
