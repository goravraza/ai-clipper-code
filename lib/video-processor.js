...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 720, sessionId = null }) {
  ...
  try {
    ...
    if (shouldFallback && failedClipIndices.length > 0) {
      try {
        ...
        fullVideoFallbackPath = await fetchFullVideoFromYouTube({ url, outDir: videoDir, db, maxHeight: 720, sessionId: proxySessionId })
