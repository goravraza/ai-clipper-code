...
export async function getProxyUrl(db, opts = {}) {
  const integ = await getIntegration(db, 'http_proxy')
  const raw = integ?.credentials?.proxy_url || process.env.HTTP_PROXY_URL || null
  // For Bright Data, inject a sticky session ID so all yt-dlp calls reuse the same exit IP.
  // This prevents YouTube 403 errors caused by IP-signed stream URLs being fetched from different exit nodes.
  // Session id is derived from a stable per-job seed (passed in opts) so all calls within one ingestion share an IP.
  return normalizeProxyUrl(raw, opts.sessionId)
}
...
// If `sessionId` is supplied AND the user is a Bright Data customer (brd-customer-...),
// appends `-session-{sessionId}` to the username for sticky-IP behaviour (critical for YouTube).
function normalizeProxyUrl(raw, sessionId = null) {
  if (!raw || typeof raw !== 'string') return null
  let s = raw.trim()
  const addSession = (user) => {
    if (!sessionId) return user
    if (!/^brd-customer-/i.test(user)) return user  // only Bright Data supports the -session- suffix
    if (/-session-/.test(user)) return user  // already has a session
    return `${user}-session-${sessionId}`
  }
  // Already standard "http://user:pass@host:port"
  const stdMatch = s.match(/^(https?:\/\/)([^:@]+):([^@]+)@(.+)$/i)
  if (stdMatch) {
    const [, scheme, user, pass, host] = stdMatch
    return `${scheme}${encodeURIComponent(addSession(user))}:${encodeURIComponent(pass)}@${host}`
  }
  ...
  if (parts.length >= 4) {
    ...
    if (host && port && user && pass) {
      return `${scheme}${encodeURIComponent(addSession(user))}:${encodeURIComponent(pass)}@${host}:${port}`
    }
  }
  return scheme + s
}
...
export async function getCookiesFile(db) {
...
async function extractStreamUrl(url, proxyUrl, cookiesFile, { audioOnly = false, maxHeight = 1080 } = {}) {
  const fmtChain = audioOnly
    ? 'bestaudio[ext=m4a]/bestaudio'
    : [
        // Prefer separate 1080p video + best audio (yt-dlp will provide a single -g URL for the video track; audio is handled downstream)
        `bestvideo[height<=${maxHeight}][ext=mp4]`,
        `bestvideo[height<=${maxHeight}]`,
        // Progressive fallbacks (video+audio in one stream, capped by height)
...
export async function fetchYouTubeSegment(url, startSec, endSec, outPath, proxyUrl, cookiesFile, maxHeight = 1080) {
  const start = Math.max(0, Math.floor(startSec))
  const end = Math.max(start + 1, Math.ceil(endSec))
  // Format selector for per-segment downloads.
  // CRITICAL: yt-dlp's `--download-sections` cut is unreliable with DASH (separate video+audio streams);
  // in some clips only the video track is preserved, silently dropping audio. So we use PROGRESSIVE
  // formats only here (single-stream files where video+audio are already muxed together upstream).
  // Full-quality DASH (bestvideo+bestaudio) is safe to use in the whole-video download path (see fetchFullVideoFromYouTube)
  // — just not with --download-sections.
  const fmt = [
    // Progressive HD → guarantees audio+video in one atomic stream that --download-sections can cut cleanly
    `best[height<=${maxHeight}][ext=mp4]`,
    `best[height<=${maxHeight}]`,
    `best[height<=720][ext=mp4]`,
    `best[height<=720]`,
  ...
  for (let i = 0; i < tiers.length; i++) {
    ...
    try {
      const args = [
        ...
        // Ensure video+audio streams (DASH) get muxed into a single mp4 container downstream.
...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 1080, sessionId = null }) {
  await fs.mkdir(outDir, { recursive: true })
  // Use sticky-session proxy (same Bright Data exit IP across all calls) to avoid YouTube 403s.
  const sid = sessionId || `dl${Date.now()}${Math.floor(Math.random() * 9999)}`
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    try {
      // Full-quality format chain — try SIMPLE progressive formats FIRST (much higher success rate on YouTube proxied downloads),
      // then fall through to DASH bestvideo+bestaudio. Progressive is a single stream so no merge is needed.
      // Adding --merge-output-format mp4 so bestvideo+bestaudio DASH gets muxed to a single .mp4 file.
      const fmtChain = [
        // Progressive first — much more likely to succeed via residential proxies (single stream, no merge)
        `best[height<=720][ext=mp4]`,
        `best[ext=mp4]`,
        `best[height<=${maxHeight}][ext=mp4]`,
        `best[height<=${maxHeight}]`,
        `best[height<=720]`,
        `best[height<=480]`,
        // DASH fallbacks (need --merge-output-format)
...
export async function getGroqWhisperConfig(db) {
  if (!db) return null
  try {
...
export async function transcribeWithWhisper(audioPath, apiKey, language = null, db = null) {
  ...
  for (const p of chain) {
    try {
      ...
      if (p.supportsSegments) {
        ...
        // Request BOTH segment- AND word-level timestamps so we can support Vizard/Opus-style
        // word-by-word karaoke highlighting during render. Groq + Whisper-large-v3 both support this.
      ...
      console.log(`[transcribe] Used ${p.name} (${elapsed}s, audio ${audioDuration.toFixed(1)}s${Array.isArray(data.words) ? `, ${data.words.length} words` : ''})`)
      if (Array.isArray(data.segments) && data.segments.length > 0) {
        // Attach word-level timings (if available) to each segment they fall inside of.
        ...
        return data.segments.map(s => {
          ...
          const segWords = allWords
            .filter(w => Number.isFinite(w.start) && Number.isFinite(w.end) && w.end > sStart - 0.05 && w.start < sEnd + 0.05)
            .map(w => ({ start: Number(w.start), end: Number(w.end), text: String(w.word || w.text || '').trim() }))
            .filter(w => w.text)
...
export async function processVideoInBackground({
  ...
  try {
    ...
    // Sticky session ID hoisted to the try-scope so both stage 1 AND the full-video fallback (if triggered) reuse it.
    // This ensures the fallback yt-dlp download hits the SAME Bright Data / Thordata exit IP as the metadata call.
    const proxySessionId = `clipforge${videoId.slice(0,8)}${Date.now().toString(36)}`
    ...
    if (localFile) {
      ...
      // YouTube signs stream URLs to the requesting IP — without sticky session, segment fetches 403.
      proxyUrl = await getProxyUrl(db, { sessionId: proxySessionId })
    ...
    const prompt = hasTranscript
      ? `You are an expert short-form video editor. Below is a timestamped transcript of a long video. Pick exactly ${targetClips} highly-engaging segments most likely to go viral on TikTok / Shorts / Reels. Each clip MUST be between ${minLen} and ${maxLen} seconds long, and start times must be SPREAD ACROSS the video (don't bunch them together).\n\n**LANGUAGE RULE**: The transcript below is in a specific language. Your response MUST use the SAME language for BOTH \`clip_title\` AND \`hook_text\` (e.g. if transcript is in Hindi/Devanagari, titles and hooks must be in Hindi Devanagari; if Spanish, titles in Spanish; etc.). NEVER translate to English unless the transcript itself is English.\n\nTRANSCRIPT:\n${transcript}\n\nReturn ONLY valid JSON, no markdown. You MUST return at least ${minClips} clips:\n{ "clips": [ { "clip_title": "punchy curiosity-gap title under 60 chars (SAME LANGUAGE as transcript)", "start_time_seconds": int, "end_time_seconds": int (must be ${minLen}-${maxLen}s after start), "virality_score": 70-99, "hook_text": "4-8 word scroll-stopping opener (SAME LANGUAGE as transcript)", "transcript_segment": "the exact text spoken during this clip", "why": "one sentence on why this will go viral (can be in English)" } ] }\nSort by virality_score desc. Return ${targetClips} clips.`
    ...
    // Robust JSON extraction — handles markdown fences, prose prefix/suffix, trailing commas,
    // and even a stray leading unicode BOM.
    ...
    const tryParse = (s) => {
      ...
      // Extract the biggest {...} JSON block
      const m = s.match(/\{[\s\S]*\}/)
      if (m) {
    ...
    if (!parsed?.clips || !Array.isArray(parsed.clips)) {
      // AI returned garbage/prose — log a sample and gracefully fall back to synthesizing clips
      // (same code path we use when the AI returns fewer clips than requested).
      console.warn(`[ai] Unparseable response, falling back to synthesized clips. First 300 chars: ${cleaned.slice(0, 300)}`)
      parsed = { clips: [] }
    }
    ...
    async function tryCutClip(c, i, opts = {}) {
      ...
      let perClipTranscriptionSource = null
      ...
      // STEP B.2: PER-CLIP TRANSCRIPTION FALLBACK
      // If the full-video transcription failed earlier (segments.length === 0 or srt is empty),
      // extract audio directly from this clip's MP4 and run Whisper on it. This handles the case
      // where YouTube's audio-only stream download is blocked but the per-clip segment download worked.
      if (!srtContentSaved) {
        try {
          const apiKey = await getOpenAIKey(db)
          const groq = await getGroqWhisperConfig(db)
          if ((apiKey || groq) && (await fileExists(finalPath))) {
            const clipAudioPath = path.join(videoDir, `clip_${clipId}.mp3`)
            await extractAudio(finalPath, clipAudioPath)
            const audioStat = await fs.stat(clipAudioPath).catch(() => null)
            if (audioStat && audioStat.size > 512) {
              const clipSegs = await transcribeWithWhisper(clipAudioPath, apiKey, language, db)
              if (clipSegs && clipSegs.length > 0) {
                // clipSegs are already in 0..clipDuration time — pass 0 and clipDuration to buildClipSrt.
                const srtLocal = buildClipSrt(clipSegs, 0, end - start)
                if (srtLocal.trim().length > 0) {
                  srtContentSaved = srtLocal
                  perClipTranscriptionSource = 'whisper_per_clip'
                  console.log(`[per-clip transcribe] ${clipId.slice(0,8)} → ${clipSegs.length} segments`)
                }
              }
            }
            try { await fs.unlink(clipAudioPath) } catch {}
          }
        } catch (perClipErr) {
          console.warn(`[per-clip transcribe] ${clipId.slice(0,8)} failed:`, perClipErr.message)
        }
      }
      ...
      const doc = {
        ...
        transcription_source: perClipTranscriptionSource || transcriptionSource,
    ...
    if (shouldFallback && failedClipIndices.length > 0) {
      try {
        ...
        fullVideoFallbackPath = await fetchFullVideoFromYouTube({ url, outDir: videoDir, db, maxHeight: 1080, sessionId: proxySessionId })
    ...
    // Persist the FULL SOURCE VIDEO + FULL TRANSCRIPT so downstream features can use them:
    //   - "Download full video" button streams the source MP4
    //   - "Create Supercut" reads segments from the full transcript and extracts from the full source
    // For uploads: copy the local file to a stable sources dir
    // For YouTube URLs: attempt a background-friendly full download (uses fetchFullVideoFromYouTube).
    // Full transcript = the `segments` array we already computed via Whisper / VTT captions.
    try {
      const SOURCES_DIR = path.join(UPLOAD_DIR, 'sources')
      await fs.mkdir(SOURCES_DIR, { recursive: true })
      const sourceOutPath = path.join(SOURCES_DIR, `${videoId}.mp4`)
      let sourceSaved = false
      if (videoPath && (await fileExists(videoPath))) {
        // Uploads: copy (not move) so downstream code that still references videoPath keeps working
        try {
          await fs.copyFile(videoPath, sourceOutPath)
          sourceSaved = true
        } catch (copyErr) { console.warn('[full-source] copy failed:', copyErr.message) }
      } else if (url && extractYouTubeId(url)) {
        // YouTube: try to download the full video (best quality up to 1080p). This is heavy — 60-200 MB typical.
        try {
          const proxySessionId2 = `clipforge${videoId.slice(0,8)}${Date.now().toString(36)}`
          const proxyUrl2 = await getProxyUrl(db, { sessionId: proxySessionId2 })
          const fullPath = await fetchFullVideoFromYouTube({ url, outDir: videoDir, db, maxHeight: 1080, sessionId: proxySessionId2 })
          if (fullPath && await fileExists(fullPath)) {
            await fs.rename(fullPath, sourceOutPath)
            sourceSaved = true
            void proxyUrl2 // silence unused-var
          }
        } catch (fullErr) {
          console.warn('[full-source] YouTube full download failed:', fullErr.message)
        }
      }
      if (sourceSaved) {
        try {
          const st2 = await fs.stat(sourceOutPath)
          await db.collection('videos_processed').updateOne({ id: videoId }, { $set: {
            source_video_path: sourceOutPath,
            source_video_size_bytes: st2.size,
          } })
        } catch (statErr) { console.warn('[full-source] stat failed', statErr.message) }
      }
      // Persist the full transcript with word-level timings (if available)
      if (Array.isArray(segments) && segments.length > 0) {
        const compact = segments.map(s => ({
          ...
          words: Array.isArray(s.words) ? s.words.map(w => ({
            ...
            text: String(w.text || w.word || '').slice(0, 40),
          })) : [],
        }))
        await db.collection('videos_processed').updateOne({ id: videoId }, { $set: {
          full_transcript_segments: compact,
          full_transcript_text: transcript || compact.map(s => s.text).join(' ').slice(0, 20000),
          transcription_source: transcriptionSource,
        } })
      }
    } catch (persistErr) {
      console.warn('[full-source] persistence failed (non-fatal):', persistErr.message)
    }
...
// On-demand per-clip transcription — used by POST /api/clips/:id/transcribe.
// Extracts audio from a single clip's local MP4 and runs Groq/OpenAI Whisper on it.
// Returns { ok, srt_content, segments_count } on success or { ok: false, error } on failure.
export async function transcribeClipOnDemand({ db, clipId, clipPath, clipDuration, language = 'auto' }) {
  try {
    const apiKey = await getOpenAIKey(db)
    const groq = await getGroqWhisperConfig(db)
    if (!apiKey && !groq) {
      return { ok: false, error: 'No transcription provider configured', hint: 'Add a Groq or OpenAI API key in Admin → Integrations.' }
    }
    if (!(await fileExists(FFMPEG))) {
      return { ok: false, error: 'ffmpeg binary not found' }
    }
    if (!(await fileExists(clipPath))) {
      return { ok: false, error: 'Clip MP4 not found on disk' }
    }
    // Extract audio to temp mp3 next to the clip
    const audioPath = clipPath.replace(/\.mp4$/i, '') + `_ondemand_${Date.now()}.mp3`
    await extractAudio(clipPath, audioPath)
    const st = await fs.stat(audioPath).catch(() => null)
    if (!st || st.size < 512) {
      try { await fs.unlink(audioPath) } catch {}
      return { ok: false, error: 'Extracted audio file is empty or too small' }
    }
    let segs = []
    try {
      segs = await transcribeWithWhisper(audioPath, apiKey, language === 'auto' ? null : language, db)
    } finally {
      try { await fs.unlink(audioPath) } catch {}
    }
    if (!Array.isArray(segs) || segs.length === 0) {
      return { ok: false, error: 'Whisper returned no segments (audio may be silence or in an unsupported language)' }
    }
    // Segments from per-clip transcription are already local time (0..clipDuration)
    const srt = buildClipSrt(segs, 0, clipDuration)
    if (!srt || !srt.trim()) {
      return { ok: false, error: 'Failed to build SRT from segments' }
    }
    console.log(`[on-demand transcribe] ${clipId.slice(0,8)} → ${segs.length} segments, srt ${srt.length} chars`)
    return { ok: true, srt_content: srt, segments: segs, segments_count: segs.length }
  } catch (e) {
