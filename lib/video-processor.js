...
async function fetchYouTubeSegment(url, startSec, endSec, outPath, proxyUrl, cookiesFile, maxHeight = 720) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (proxyUrl) args.push(`--proxy=${proxyUrl}`)
      args.push(url)
      console.log(`[ytseg attempt ${attempt}] ${YT_DLP} ${args.map(a => a.length > 50 ? a.slice(0,30)+'...' : a).join(' ')}`)
      const env = { PATH: `/root/.deno/bin:/root/.venv/bin:${process.env.PATH || ''}` }
      ...
      lastErr = e
      console.warn(`[ytseg attempt ${attempt}] failed: ${e.message.slice(0,200)}`)
...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 720 }) {
  ...
  const args = ['-f', `best[height<=${maxHeight}][ext=mp4]/best[height<=${maxHeight}]/best[ext=mp4]/best`, '--no-playlist', '--no-warnings', '-o', outTemplate]
  if (cookiesFile) args.push('--cookies', cookiesFile)
  if (proxyUrl) args.push(`--proxy=${proxyUrl}`)
  args.push(url)
  const env = { PATH: `/root/.deno/bin:${process.env.PATH || ''}` }
  ...
  const f = files.find(n => /\.(mp4|webm|mkv)$/.test(n))
...
export async function processVideoInBackground({
  ...
  try {
    ...
    for (let i = 0; i < parsed.clips.length; i++) {
      ...
      // STEP A: Cut raw clip — three branches
      ...
      try {
        if (videoPath) {
          ...
          // YouTube — use yt-dlp --download-sections (proxy + cookies aware)
          // Skip cookies if proxy is set (proxy+cookies confuses YouTube bot detection)
          ...
          // Non-YouTube stream URL — ffmpeg directly
