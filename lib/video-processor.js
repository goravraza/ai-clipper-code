  ...
  try {
    ...
    async function tryCutClip(c, i, opts = {}) {
      ...
      if (sourceLocal) {
        ...
        // Use BOTH proxy AND cookies — YouTube's bot detection requires session cookies even from
        // residential IPs once it's seen the IP fail enough times. Cookies alone won't bypass
        // the proxy block, and proxy alone trips bot detection.
