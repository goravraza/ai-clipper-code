...
// Default subtitle style — large bold white text with black outline, bottom-center
function ffmpegSubtitleFilter(srtPath) {
  // Escape colons & special chars for ffmpeg filter
  const escaped = srtPath.replace(/\\/g, '/').replace(/:/g, '\\:').replace(/'/g, "\\'")
  const style = "FontName=DejaVu Sans,FontSize=18,PrimaryColour=&H00FFFFFF&,OutlineColour=&H00000000&,BorderStyle=1,Outline=2,Shadow=0,Bold=1,Alignment=2,MarginV=40"
    ...
    for (let i = 0; i < parsed.clips.length; i++) {
      ...
      if (burnCaptions && segments.length > 0) {
        ...
        if (srtContent.trim().length > 0) {
          ...
          try {
            await runCmd(FFMPEG, ['-y', '-i', rawClipPath, '-vf', ffmpegSubtitleFilter(srtPath), '-c:v','libx264','-preset','veryfast','-crf','22','-c:a','copy','-movflags','+faststart', clipPath], { timeout: 180000 })
            finalPath = clipPath
            captionsBurned = true
