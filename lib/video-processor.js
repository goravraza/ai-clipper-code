...
// Retries WITHOUT proxy if the proxy returns 403/connection-fail (Thordata sometimes blocks).
async function fetchYouTubeMetadata(url, proxyUrl, cookiesFile) {
  const env = { PATH: `/root/.deno/bin:${process.env.PATH || ''}` }
  for (const useProxy of [!!proxyUrl, false]) {
    if (!useProxy && !proxyUrl) {} // first pass already had no proxy
    ...
    try {
      ...
      console.log(`[meta] success ${useProxy ? '(via proxy)' : '(direct)'}`)
      return JSON.parse(stdout)
    } catch (e) {
      console.warn(`[meta] ${useProxy ? 'via-proxy' : 'direct'} failed: ${e.message.slice(0,160)}`)
      if (!useProxy) throw e  // both attempts failed
    }
  }
  throw new Error('metadata fetch failed (no fallback)')
}

// Step 2: get a directly-fetchable stream URL signed for our outbound IP.
// Same proxy-then-direct fallback as metadata.
async function extractStreamUrl(url, proxyUrl, cookiesFile, { audioOnly = false, maxHeight = 720 } = {}) {
  ...
  for (const useProxy of [!!proxyUrl, false]) {
    ...
    try {
      ...
      if (lines[0]) { console.log(`[stream-url] resolved ${useProxy ? 'via-proxy' : 'direct'}`); return lines[0] }
    } catch (e) {
      console.warn(`[stream-url] ${useProxy ? 'via-proxy' : 'direct'} failed: ${e.message.slice(0,160)}`)
      if (!useProxy) throw e
    }
  }
  return null
...
async function fetchYouTubeSegment(url, startSec, endSec, outPath, proxyUrl, cookiesFile, maxHeight = 1080) {
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    try {
      ...
      if (cookiesFile) args.push('--cookies', cookiesFile)
      // Try proxy on attempts 1-3; fall back to direct (no proxy) on attempts 4-5 in case proxy 403s
      if (proxyUrl && attempt <= 3) args.push(`--proxy=${proxyUrl}`)
      args.push(url)
      console.log(`[ytseg attempt ${attempt}/5${attempt > 3 ? ' (no-proxy)' : ''}] ${YT_DLP} ${args.slice(-3).join(' ')}`)
...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 720 }) {
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    ...
    try {
      ...
      if (useProxy) args.push(`--proxy=${proxyUrl}`)
      if (cookiesFile) args.push('--cookies', cookiesFile)
      args.push(url)
      const env = { PATH: `/root/.deno/bin:/root/.venv/bin:/usr/local/bin:/usr/bin:/bin:${process.env.PATH || ''}` }
      console.log(`[full-dl attempt ${attempt}/4${useProxy ? '' : ' (direct)'}] yt-dlp ${url.slice(0,60)}`)
