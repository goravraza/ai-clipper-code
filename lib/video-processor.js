...
async function transcribeWithWhisper(audioPath, apiKey, language = null) {
  ...
  for (const model of [primaryModel, 'whisper-1']) {
    try {
      ...
      f.append('file', blob, 'audio.mp3')
      f.append('model', model)
      // gpt-4o-transcribe doesn't yet support 'verbose_json' or segment granularities.
      // whisper-1 supports both. Use the right format per model.
      if (model === 'whisper-1') {
        f.append('response_format', 'verbose_json')
        f.append('timestamp_granularities[]', 'segment')
      } else {
        // gpt-4o-transcribe: ask for plain JSON; we'll build segments from the response if available
      ...
      console.log(`[transcribe] Used model: ${model}`)
      // whisper-1 returns { segments: [{start, end, text}] }
      if (Array.isArray(data.segments) && data.segments.length > 0) {
        return data.segments.map(s => ({ start: Number(s.start) || 0, end: Number(s.end) || 0, text: String(s.text || '').trim() })).filter(s => s.text)
      }
      // gpt-4o-transcribe returns { text: "..." } without timestamps.
      // Synthesize evenly-spaced segments so caption burn-in still works (rough timing).
      if (typeof data.text === 'string' && data.text.trim().length > 0) {
        const fullText = data.text.trim()
        // chunk by ~6 words for typeset readability
        const words = fullText.split(/\s+/).filter(Boolean)
        const chunkSize = 6
        const numChunks = Math.max(1, Math.ceil(words.length / chunkSize))
        // We don't have audio duration here — caller will trim with real clip bounds. Use 1s/chunk approx.
        // Actual duration is fixed up by the per-clip srt builder which clamps to clipEnd-clipStart.
        const segs = []
        for (let i = 0; i < numChunks; i++) {
          const txt = words.slice(i * chunkSize, (i + 1) * chunkSize).join(' ')
          segs.push({ start: i, end: i + 1, text: txt })
        }
        return segs
      }
