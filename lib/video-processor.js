...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 1080, sessionId = null }) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    try {
      // Full-quality format chain — prefer separate 1080p video+audio (DASH), fall through progressive tiers.
      const fmtChain = [
        `bestvideo[height<=${maxHeight}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=${maxHeight}]+bestaudio`,
        `best[height<=${maxHeight}][ext=mp4]`,
        `best[height<=${maxHeight}]`,
        `best[height<=720][ext=mp4]`,
        `best[height<=720]`,
...
export async function processVideoInBackground({
  ...
  try {
    ...
    if (localFile) {
      ...
      // Sticky session ID — shared across ALL yt-dlp calls for this ingestion so we hit the same Bright Data exit IP.
