...
async function fetchYouTubeSegment(url, startSec, endSec, outPath, proxyUrl, cookiesFile, maxHeight = 720) {
...
export async function fetchFullVideoFromYouTube({ url, outDir, db, maxHeight = 720 }) {
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      ...
      // Cookies + proxy together confuse YouTube — pick one
      if (proxyUrl) args.push(`--proxy=${proxyUrl}`)
...
async function transcribeWithWhisper(audioPath, apiKey, language = null) {
  ...
  // Probe audio duration so we can scale synthetic segments from gpt-4o-transcribe
  // (gpt-4o-transcribe returns only {text} with no per-segment timestamps).
  let audioDuration = 0
  try {
  ...
  // gpt-4o-transcribe: newer OpenAI ASR — significantly better for Hindi/Indic languages than whisper-1.
  // Falls back to whisper-1 automatically if the gpt-4o model is unavailable for the key.
  const primaryModel = process.env.OPENAI_TRANSCRIBE_MODEL || 'gpt-4o-transcribe'
  let lastErr = null
  for (const model of [primaryModel, 'whisper-1']) {
    try {
      const f = new FormData()
      const blob2 = new Blob([audioBuf], { type: 'audio/mpeg' })
      f.append('file', blob2, 'audio.mp3')
      f.append('model', model)
      // whisper-1 supports verbose_json + segment timestamps. gpt-4o-transcribe does NOT (only plain JSON).
      if (model === 'whisper-1') {
      ...
      const r = await fetch('https://api.openai.com/v1/audio/transcriptions', { method: 'POST', headers: { authorization: `Bearer ${apiKey}` }, body: f })
      if (!r.ok) {
        const txt = await r.text().catch(()=>'')
        lastErr = new Error(`${model} ${r.status}: ${txt.slice(0,200)}`)
        console.warn(`Transcribe failed with ${model}, trying next:`, lastErr.message)
        continue
      }
      const data = await r.json()
      console.log(`[transcribe] Used model: ${model} (audio ${audioDuration.toFixed(1)}s)`)
      // whisper-1: real per-segment timestamps
      if (Array.isArray(data.segments) && data.segments.length > 0) {
        return data.segments.map(s => ({ start: Number(s.start) || 0, end: Number(s.end) || 0, text: String(s.text || '').trim() })).filter(s => s.text)
      }
      // gpt-4o-transcribe: only plain text — synthesize segments by distributing words evenly over audio duration.
      // This way they line up with the source video timeline and buildClipSrt can extract proper per-clip cues.
      if (typeof data.text === 'string' && data.text.trim().length > 0) {
        const fullText = data.text.trim()
        const words = fullText.split(/\s+/).filter(Boolean)
        if (words.length === 0) { lastErr = new Error(`${model}: no words`); continue }
        const wordsPerCue = 6
        const numCues = Math.max(1, Math.ceil(words.length / wordsPerCue))
        const totalDur = audioDuration > 0 ? audioDuration : numCues * 2  // 2s/cue fallback if probe failed
        const cueDur = totalDur / numCues
        const segs = []
        for (let i = 0; i < numCues; i++) {
          const txt = words.slice(i * wordsPerCue, (i + 1) * wordsPerCue).join(' ')
          if (!txt) continue
          segs.push({ start: i * cueDur, end: (i + 1) * cueDur, text: txt })
        }
        if (segs.length > 0) {
          console.log(`[transcribe] Synthesized ${segs.length} segments across ${totalDur.toFixed(1)}s from gpt-4o text`)
          return segs
        }
      }
      ...
      lastErr = e
      console.warn(`Transcribe failed with ${model}:`, e.message)
    }
  }
...
export async function processVideoInBackground({
  ...
  try {
      ...
      if (apiKey && (await fileExists(FFMPEG))) {
        try {
          ...
          if (videoPath) {
            ...
            // Use yt-dlp to download audio-only directly (it handles proxy redirects properly,
            // unlike ffmpeg+stream URL which trips TLS errors on Thordata exit nodes).
            try {
    ...
    async function tryCutClip(c, i, opts = {}) {
      ...
      if (sourceLocal) {
        await runCmd(FFMPEG, ['-y', '-ss', String(start), '-i', sourceLocal, '-t', String(clipDuration), '-c:v','libx264','-preset','veryfast','-crf','24','-c:a','aac','-b:a','128k','-movflags','+faststart', rawClipPath], { timeout: 120000 })
      } else if (extractYouTubeId(url)) {
        const cookiesArg = proxyUrl ? null : await getCookiesFile(db)
        await fetchYouTubeSegment(url, start, end, rawClipPath, proxyUrl, cookiesArg, 720)
      } else if (streamUrl) {
      ...
      // STEP B: Optionally burn captions
